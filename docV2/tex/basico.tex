...
\section{Estructura general de la biblioteca}
  ...
  \subsection{Los procesadores virtuales}
    ...
    Este tipo de paralelismo, denominado por las siglas SIMD\footnote{instrucción única multiples datos, por sus siglas en inglés}, 
    se expone en la sección \ref{flynn}. Resumiendo, se trata de aplicar una única intrucción a un conjunto de datos «en bloque».
    ...
    dificilmente portable a otros sistemas. Asimismo, su utilización suele requerir el trabajo en lenguaje ensamblador, 
    con todo lo que ello supone (dificultad de mantenimiento, complejidad, etc.).

   Para poder disponer de la potencia de esta característica presente de una forma u otra practicamente
   todos los procesadores actuales,  pese a lo poco amigable de su uso, se ha desarrollado esta «CPU SIMD». Sus 
   objetivos son:
   \begin{itemize}
      ...
      alguna, la biblioteca provee una implementación generica que simula su comportamiento.
      \item Homogeneizar la familia de operaciones disponibles, del mismo modo que se ha hecho con la CPU escalar.
      \item Proporcionar una abstracción adecuada que evite lidar con los entresijos del lenguaje ensamblador. 
   \end{itemize} 
   ...
   \item Cuantro números en coma flotante de $32$ bits.

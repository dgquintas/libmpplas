...
\section{Estructura general de la biblioteca}
  Siguiendo la filosofía de la antigua biblioteca LibNumth, se ha matenido el diseño jerárquico en capas,
  ...
  \subsection{Los procesadores virtuales}
    La biblioteca utiliza una serie de lo que se ha denominado «procesadores virtuales»: 
   ...
   al mundo de los $64$ bits en la sección \ref{impl:64bits}.
    ...
    \subsubsection{La CPU vectorial}\label{basic:cpuvectorial}
      ...
      de operar sobre vectores de \texttt{Digit}s. No existe aún concepto de signo.
  ...
  \subsection{Control de errores}\label{controlDeErrores}
    ...
    El segundo de estos puntos se desgrana a continuacion.
  ...
  \subsection{Información del sistema}\label{desc:systeminfo}
    ...
    Se ha procurado que los datos proporcionados tengan un formato fácilmente utilizable tanto como por la
    biblioteca en sí, como por un usuario humano: allí donde se utilizan descripciones textuales, éstas
    ...
    \begin{itemize}
      ...
      \item Arquitectura para la cual se ha compilado la biblioteca. Esta no tiene por que correspoder 
      necesariamente con la arquitectura del sistema donde se está utilizando la biblioteca (caso de
          una compilacion para $32$ bits sobre un sistema x86-64).
      ...
      \item Línea de comando utilizada en para invocar el compilador sobre los distintos ficheros fuente. 
    ...
    resultan aún más útiles, ya que en principio el usuario no tiene por qué saber en que sistema
    remoto se está ejecutando el servidor, es decir, la biblioteca. Los datos proporcionados resultan
    en situaciones como esta inestimables, ya que son imprescindibles a la hora de interpretar resultados
    de rendimiento o de disponibilidad de mecanismos (paralelismo, perfilado...).

    \subsubsection{Información de la CPU: \texttt{CPUInfo}}
      ...
      Este clase proporciona la siguiente información:
      \begin{itemize}
        ...
        \item Tamaño en bits de su tipo de trabajo básico (típicamente $32$ o $64$).
  ...
  \subsection{Mecanismo de perfilado}
    ...
    \cite{miproyecto}, sección 5.2.1). En esta nueva versión, este soporte se mantiene, 
    ...
    generado automáticamente por medio de una técnica conocida como AOP (Aspect Oriented Programming).
    Estos detalles se recogen en la sección \ref{impl:aop}.
      ...
      pueden abordarse la situación de dos maneras: creando efectivamente dicho nuevo grupo 
      ...
      utilizando un sistema con numeras unidades de ejecución, la segunda opción es la utilizada:
      ...
      El problema resulta cuando se tiene en cuenta lo siguiente, referido a la función \texttt{omp\_get\_thread\_num()}
      utilizada para inspeccionar el identificador del hilo ejecutandose (\cite{openmpstandard}):
      \begin{quote}
      ...
      impresión de que el método no está siendo ejecutando en paralelo si sólo se tiene en cuenta
      ...
      rendimiento, pero arrojaría unos resultado de perfilados precisos. Para activar este soporte, 
      véase \cite{openmpstandard}, secciones 3.2.9 y 4.4.
...
Versión sin optimizaciones, con la generación de símbolos de depuración y 
comprobaciones extra (de límites y asertos) activados, perfilado activado
y OpenMP desactivado. Esta es una configuración habitual durante el desarrollo
ya que facilita la depuración.
\begin{verbatim}
scons -D all enableDebug=1 enableRelease=0 optLevel=0 enableAOPProf=1 enableOpenmp=0
\end{verbatim}

Para un rendimiento máximo, se desactivan las comprobaciones extras y el perfilado;
se utiliza el máximo nivel de optimización así como las optimizaciones extra:
\begin{verbatim}
scons -D all enableRelease=1 optLevel=3 enableAOPProf=0 enableExtraOpt=1
\end{verbatim}

Es posible especificar el compilador a utilizar mediante la variable de entorno \texttt{CXX},
en este caso el compilador de Intel para C++, \texttt{icpc}:
\begin{verbatim}
CXX="icpc" scons -D all
\end{verbatim}

Especificación de la arquitectura. Por ejemplo, compilar una versión de $32$ bits de la biblioteca

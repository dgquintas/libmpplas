...
\section{Motivación}
  ...
  algo fácil de apreciar a primera vista. Puede que, y ojalá así sea,
  ...
  En términos más concretos, se busca proporcionar una manera de
    ...
    \paragraph{Características}\label{servidor:caracteristicas}
      \begin{itemize}
        ...
        correspondencia con estos, en cuanto a su definición, es prácticamente directa:
              \begin{description}
                ...
                \item[Zn:]Enteros modulares con módulo $n$ arbitrario. Se corresponde con \texttt{mpplas::Z\_n}.
      ...
      \subsubsection{Utilización de XML-RPC}
        ...
        HTTP, sobre el que XML-RPC descansa, esté orientado a transacciones (véase \cite{stallings},
          pp. 676 y siguientes, para una descripción de este protocolo), siendo éstas \emph{independientes}
  ...
  \subsection{\emph{Un} cliente}\label{cliente}
    ...
    \paragraph{Características}
      \begin{itemize}
        ...
        \item Multihilo, pudiendo ser explotado explícitamente por el usuario.
    ...
    \subsubsection{Requisitos y ejecución}
      ...
      \begin{description}
        \item[Python.] El propio entorno de ejecución. El desarrollo y las pruebas se han 
        realizado sobre la versión 2.5 del mismo.
      ...
      Para lanzar el cliente, basta con ejecutar mediante el intérprete de Python el fichero 
      \texttt{MPPLab.pyw}. La manera de invocar dicho intérprete varía 
      en función del sistema, tipo Windows o UNIX. En cualquier caso, si es invocado de forma
      textual mediante una línea de comando, se admite como parámetro el fichero de configuración
      a utilizar. De no proporcionarse ninguno, se utiliza \texttt{config.cfg}.
    ...
    \subsubsection{\textit{Bootstrapping} y autoescritura.}\label{boostrapping}
      ...
      genérica, que es instanciada con el nombre y los datos concretos de cada método, siendo
      ...
      \paragraph{¿Cómo manejar el paso de parámetros de forma genérica?}
        No todos los métodos requieren el mismo número de parámetros. Algunos siquiera
        requieren parámetros en absoluto. Sin embargo todos los métodos se están declarando,
        ...
        declara utilizando esta sintaxis, podrá recibir un número arbitrario 
        de argumentos.
      ...
      \paragraph{¿Cómo manejar los resultados de los métodos?}
        ...
        tipo de los expuestos anteriormente en el punto \ref{servidor:caracteristicas},
...
\end{lstlisting}
  ...
  de Python \texttt{ConfigParser}\footnote{\cite{pythonlib}, sección 9,2}. 
    ...
    podido ser mucho más simple, y con toda seguridad mucho menos
    potente. 
...
\end{verbatim}
      ...
      \paragraph{Ejecutando el contenido de un fichero.} La función \texttt{runFile} permite la ejecución del contenido
        del fichero cuyo nombre se le pasa como parámetro. Dicho fichero es ejecutado línea por línea del mismo modo que 
        si éstas fueran introducidas manualmente por el usuario en el interfaz.
...
\end{verbatim}
       ...
       \paragraph{Zn} Representa un entero modular de precisión arbitraria. El constructor en este caso requiere
...
\end{verbatim}
       ...
       \begin{description}
        ...
        \item[\texttt{getPBRString()}] Devuelve una cadena de caracteres representando el polinomio correspondiente
...
\end{verbatim}
       ...
       y la transposición.
       Sin pérdida de generalidad, véase el siguiente ejemplo para una matriz sobre los enteros:
       \begin{description}
         \item[\texttt{getTranspose()}] Devuelve la transpuesta de la matriz sobre la que se aplica el método.
         \item[\texttt{getDeterminant()}] Devuelve el determinante de la matriz sobre la que se aplica el método.
       \end{description}
\begin{verbatim}
...
\subparagraph{Obtención de submatrices.}
 La operación de acceso a una submatriz opera de la siguiente manera:
  \begin{description}
    ...
    del intervalos son \emph{incluidas}. 
    
    Es posible omitir estos parámetros, pero «manteniendo el orden»; es decir, es posible omitir \texttt{fin} y \texttt{paso}, pero no
    es posible omitir \texttt{paso} y especificar \texttt{fin}. Los valores dados a estos tres parámetros por omisión son $0$, $1$ y 
...
resulta posible, mediante la adecuada combinación de las operaciones de concatenación, replicar
...
sentencias se evalúan de derecha a izquierda:
\begin{verbatim}
>>> m1 = MZ("[1 2; 3 4]")
>>> print m1.appendByColumns(m2).appendByColumns(m1)
[  1  2 ]
...
\end{verbatim}
       ...
       sobre enteros son directamente aplicables a matrices sobre reales, con la salvedad, claro está, de que 
...
\end{verbatim}
    ...
    Esta función, cuyo prototipo es \texttt{crt( listaCongruencias, listaModulos )}\footnote{«CRT» es la abreviatura
      ...
      congruente con cada entero de \texttt{listaCongruencias} módulo el entero correspondiente a la misma posición de 
      \texttt{listaModulos}. 
...
\end{verbatim}
    ...
    \subsubsection{Ejecución en paralelo}\label{runInParallel}
      ...
      instrucciones a ejecutar como de un descriptor de fichero abierto para lectura, del cual se irán obteniendo
...
\end{verbatim}
      ...
      \begin{itemize}
        ...
        al definir el contenido de una matriz. 
...
\subsubsection{Ejemplos} \label{mpplab:ejs}
  \paragraph{La función $\pi(n)$.} Esta función se define, para un entero $n$, como el número de primos menores o iguales
  que $n$. En otras palabras, «cuenta primos». Debido a que se desconoce la distribución de los números primos --cuestión
  relacionada con la mítica hipótesis de Riemann--, existen formulaciones que \emph{aproximan} su valor. La más sencilla
  de estas aproximaciones resulta la dada por Gauss en el siglo XVIII, $\pi(n) \approx x / \ln(x)$.
  Sin embargo, si se desea obtener su valor exacto, no queda más remedio que efectivamente contar primos.

  En el listado \ref{lst:funcpi} se ilustra la utilización de los métodos de la biblioteca, conjuntamente con las funcionalidades
  disponibles en el cliente de MPPLab, para el cálculo de esta función.

\begin{lstlisting}[captionpos=b,basicstyle=\small,frame=shadowbox,rulesepcolor=\color{black},language=python,caption=$\pi$ de Euler,label=lst:funcpi] 
...
xlabel(r"$n$")      (*@\label{algeuler:0}@*)
ylabel(r"$\pi(n)$")
plot(t1, piEulerResList, 'ro-', label=r"True pi(n)")
plot(t1, t1/log(t1), 'bx-', label=r"n/log(n) approx.") 
legend() (*@\label{algeuler:1}@*)
\end{lstlisting}

  La particularidad de este ejemplo es el uso de los mecanismos de trazado de gráficas incorporadas
  en el cliente. Esta funcionalidad se concentra en las líneas \ref{algeuler:0}--\ref{algeuler:1}, cuyos
  resultados se representan en la figura \ref{fig:plotExResult}
  \begin{figure}[h!]
    \centering
  ...
  \paragraph{RSA y paralelismo.} El código presentado en el listado \ref{lst:rsampplab} ilustra un
    escenario real en el que se explota la función \texttt{runInParallel}: la generación del par de primos
    necesaria durante la creación de las claves pública y privada del algoritmo RSA. En concreto, se utiliza 
    en las líneas \ref{algrsa:0}--\ref{algrsa:1}. El resto del código es asimismo un buen ejemplo de lo sencillo
    que resulta la implementación del algoritmo RSA en el entorno MPPLab.
\begin{lstlisting}[captionpos=b,basicstyle=\small,frame=shadowbox,rulesepcolor=\color{black},language=python,caption=RSA paralelo con MPPLab,label=lst:rsampplab] 
from Client import *
def getKeyPair(keySize):
  " Returns the pair (public, private) keys"
  (p,q) = runInParallel(["result=getPrime(%d)" % keySize ]*2, (*@\label{algrsa:0}@*)
      2, 
      "Generating prime pair (p,q)")  (*@\label{algrsa:1}@*)
  n = p*q
  phi = (p-1)*(q-1)
  while True:
...
\end{lstlisting}
Si se supone que el código del listado está contenido en \texttt{RSA.py}, se presenta a continuación un ejemplo 
de uso --en el que se utiliza el mecanismo de módulos de Python, véase \cite{pythonref}\footnote{sección 6.12}--:
\begin{verbatim}
>>> import RSA
>>> (privateKey, publicKey) = RSA.getKeyPair(64)
>>> privateKey
(454145867573780626450634936227269767, 891090725477588234798495840189752147)
>>> cyphertext = RSA.cypher(publicKey, Z("12345"))
>>> cyphertext
197459952109445108444877291109778841
>>> cleartext = RSA.decypher(privateKey, cyphertext)
>>> cleartext
12345
\end{verbatim}

  \paragraph{Generadores y $\pi$.}
    Cambiando de tercio, se abandona el dominio de la Teoría de Números para dar paso a un ejemplo
    relacionado con los números reales. 

    En este ejemplo explota una característica de Python muy interesante, sobre todo a la hora de
    trabajar con métodos iterativos que van refinando su respuesta en iteraciones sucesivas: los generadores. 
    Éste es un mecanismo propio del lenguaje el cual, por brevedad, no será descrito aquí en profundidad: 
    véase \cite{pepgenerators} para más detalles. De forma muy resumida, un generador es una función capaz
    de deternese en un punto arbitrario de su definición, devolver un valor, y posteriormente reanudar
    su ejecución exactamente desde el mismo punto, y con el mismo estado, en el que se había detenido, como
    si nada hubiera pasado. Entre otras muchas, la representación de series infinitas es una de sus aplicaciones.
    Puede considerarse que un generador representa una «ventana» a una lista de valores infinita, sin necesidad
    de almacenar dicha lista (por suerte, ya que es infinita) en ningún lugar. 

    Por ejemplo, considérese la formula de Leibniz para el cálculo de la constante $\pi$:
    \begin{equation}\label{eq:pileibniz}
        \sum_{n=0}^{\infty} \frac{(-1)^{n}}{2n+1} = \frac{1}{1} - \frac{1}{3} + \frac{1}{5} - \frac{1}{7} + \frac{1}{9} - \cdots = \frac{\pi}{4}
    \end{equation}
    La codificación de esta fórmula se da en la función generador \texttt{pi\_series} del listado \ref{lst:pimpplab}. Cada vez que
    se solicita un nuevo valor al generador, se procede con un elemento adicional de la anterior serie \eqref{eq:pileibniz}.
    Yendo un paso más allá, es posible definir un nuevo generador sobre este generador, con el fin de acelerar la convergencia.
    El siguiente método de aceleración de la convergencia se debe a Euler:
    \[
    S_{n+1} - \frac{(S_{n+1} - S_{n})^2}{S_{n-1} - 2S_n + S_{n+1}}
    \]
    donde $S_n$ representa el término $n$-ésimo de la serie original. Este acelerador
    se implementa en la función generador \texttt{euler\_accelerator}.

\begin{lstlisting}[captionpos=b,basicstyle=\small,frame=shadowbox,rulesepcolor=\color{black},language=python,caption=Generando la constante $\pi$,label=lst:pimpplab] 
from Client import *
def pi_series():
  sum = R('0')
  i = R('1.0')
...
\end{lstlisting}
    ...
    Supóngase que el código dado en el listado \ref{lst:pimpplab} está contenido en un fichero de nombre
    \texttt{piSeries.py}. A continuación se representa parte de una sesión en la cual se utiliza este código:
\begin{verbatim}
>>> runFile("piSeries.py")
>>> g = pi_series()
>>> gg = euler_accelerator(g)
>>> list( firstn(gg, 15) )
[3.16666666666666666667, 3.13333333333333333333, 3.14523809523809523810, 
 3.13968253968253968254, 3.14271284271284271284, 3.14088134088134088134, 
 3.14207181707181707182, 3.14125482360776478424, 3.14183961892940221114, 
 3.14140671849650177824, 3.14173609926066515109, 3.14147968900425489468, 
 3.14168318920775509818, 3.14151898559527562364, 3.14165339419742616127]
>>> gg.next()
3.14154198599778266751
>>> gg.next()
3.14163535667938864323
>>> gg.next()
3.14155633028457277473
\end{verbatim}
La sentencia \texttt{list( firstn(gg, 15) )} genera una lista formada por quince
iteraciones del método acelerador de la convergencia. Nótese que tanto la variable
\texttt{g} como \texttt{gg} son generadores, del algoritmo original \texttt{pi\_series}
y del algoritmo acelerador de la convergencia \texttt{euler\_accelerator} respectivamente.
Es por tanto posible utilizar los métodos propios de las instancias de estos objetos, como
se ilustra en el resto de las sentencias. Internamente, los tipos de dato reales utilizados 
son de la biblioteca, aunque se abstraiga de este detalle al usuario.

...
\section{Introducción}
  ...
  \begin{description}
    ...
    métodos secuenciales no sea posible sin una reescritura completa del método. Nótese sin embargo cómo,
...
\section{Producto de enteros}\label{par:karatsubaZ}
 ...
 continuación la formulación de este método.
  ...
  \end{equation}
   ...
   Los sumandos de la expresión \eqref{eq:multiplicacionKaratsuba} conforman tres tareas
   totalmente independientes entre sí, siendo por tanto \emph{embarazosamente paralelizable}.

   Se muestra el método resultante en el algoritmo \ref{alg:karatsuba}, en el cual se distinguen las 
  ...
  \subsection{Comparativa}
    En la figura \ref{fig:karatsuba} se compara la multiplicación de enteros
...
\section{Potenciación modular}
  ...
  versión de esta biblioteca: véase \cite{miproyecto}\footnote{sección 5.5.3, pág. 64}. Es fácil observar cómo este
  ...
  Debido a que el anterior método dejaba que desear, sobre todo en cuanto a escalabilidad, hemos
 ...
 \subsubsection{Análisis de complejidad}
  Sea $T(e)$ el tiempo requerido para un exponente $e$, $t = bitLength(e)$ y $wt =$ número de unos en $e$. 
...
\section{Vectores}
  \subsection{Producto escalar}
    ...
    elemementos con identico índice son totalmente independientes las unas de las otras.

    Los resultados de la paralelización se muestra en las figuras \ref{fig:resDot} y 
    ...
    \begin{figure}[h]
      ...
      \caption{Comparativa producto escalar (proporción)}\label{fig:propDot}
    \end{figure}
    Estos resultados no son especialmente brillantes, debido en gran medida a que la segunda
    parte del cálculo del producto escalar, el cálculo del sumatorio sobre los resultados
    de las multiplicaciones, no se realiza en paralelo. 

  \subsection{Instrucciones SIMD}
    Si el cálculo del producto escalar se realiza sobre alguna de las instancias 
    concretas que utilizan la CPU SIMD, descritas en la sección \ref{basico:cpusimd}, 
    se estará aprovechando también otra clase de paralelismo.

    Por ejemplo, considérense vectores del tipo \texttt{VectorFloat}, formados por instancias
    del tipo básico \texttt{float} empaquetadas de cuatro en cuatro. La máxima ganancia teórica,
    sería por tanto de $4$, ya que estos paquetes de cuatro elementos se procesan simultaneamente,
    ...
    Pese a ser una ganancia nada despreciable, es considerablemente inferior al teórico $4$: principalmente 
    contenciones en el bus del sistema rebajan la ganancia al resultado práctico obtenido.
    \begin{figure}[h]                                                 
      \centering                                                      
    ...
    \begin{figure}[h]                                                 
      ...
      \caption{Comparativa SIMD (proporción)}\label{fig:propSIMD}
    \end{figure}                                                      
                                                                      
                                                                      
\section{Matrices}                                                    
  \subsection{Producto}\label{par:matprod}                            
    La paralelización del producto matricial ha sido objeto de un profundo estudio prácticamente
    desde el nacimiento del concepto de computación paralela. Desde un punto de vista secuencial,
    la implementación del producto de matrices en la biblioteca se reduce a la multiplicación de dos
    matrices $2 \times 2$, fruto del uso del algoritmo de Strassen-Winograd (véase la sección
    \ref{impl:producto}). Esto resulta en el patrón de accesos representado en la figura \ref{fig:patronProd}.
    ...
    Es decir, es posible paralelizar el bucle más externo, tal como se muestra en el algoritmo
    \ref{alg:prodMatPar}.
    \begin{algorithm}
  ...
  \subsubsection{Comparativa}
    ...
    Este resultado no resulta brillante, pero tampoco podría calificarse como malo. El proceso
    de paralelización en sí es dificilmente mejorable, ya que la división en tareas se realiza
    en el bucle más externo, con lo que se minimiza la componente secuencial. Sin embargo, la 
    imposibilidad de realizar un particionado acorde con el tamaño de las cachés, tal como ya 
    se ha expuesto con anterioridad, repercute negativamente en el rendimiento. La 
    figura \ref{fig:prodMat} muestra --tanto para la ejecución secuencial como paralela-- los
    «dientes de sierra» propios de los métodos con particionado con «valores de frontera»
    entre las partes.
  ...
  \subsection{Inversión}
    El proceso de inversión, descrito en la sección \ref{tipos:invMat}, se reduce a la resolución
    de $n$ sistemas de ecuaciones para una matriz en $\mathcal{M}_{n}$. Estos sistemas, y así su
    resolución, son totalmente independientes los unos de los otros, resultando en un problema
  ...
  \subsection{Determinante}
    \subsubsection{Algoritmo de Gauss-Bareiss}\label{para:bareiss}
      ...
      De este patrón de acceso se deriva una paralelización organizada según el acceso a los datos y
    ...
    \paragraph{Comparativa}
        Se presenta en este método, de una forma mucho más acusada que en el resto, la circunstancia
        de que el método paralelo sólo empiece a obtener mejores tiempos para matrices relativamente 
        grandes. En la figura \ref{fig:propBarreis}, y con mayor detalle en la figura \ref{fig:propBarreisDetalle},
        se aprecia como la gráfica corta a $y = 1$ para unas dimensiones de matriz de $50$. Para dimensiones
        menores, el método secuencial se ejecuta en menos tiempo.
      ...
      \begin{figure}[h] 
        ...
        \caption{Comparativa Gauss-Bareiss (proporción)}
      \end{figure}

      Considerando la proporción «asintótica» derivada de esta figuras, se extrae una 
      ganancia de $1/0.81 \approx 1.23$, y una eficiencia de $1.23 / 2 = 0.615$.


\section{Cálculo de \textit{reducciones}}
  En el contexto de la programación funcional, una función de \emph{reducción} se corresponde con el
  procesamiento en un orden y de manera sistemática de una estructura de datos, resultándo 
  en un único valor. Es decir, la estructura de datos original es \emph{reducida} a un único dato.
  Estas funciones se componen fundamentalmente de dos elementos: una función de combinación (típicamente binaria) y 
  la estructura de datos a procesar (típicamente una lista/vector). 
  En el ámbito matemático, dos ejemplos habituales son el sumatorio y el productorio. Para estos dos ejemplos, 
  las operaciones serían, respectivamente, la suma y el producto. 

  El cálculo paralelo de reducciones está directamente soportado por OpenMP mediante la clausula \texttt{reduce} 
  (\cite{openmpstandard}\footnote{sección 2.8.3.6}). Por desgracia, sólo soporta reducciones sobre tipos básicos
  de C/C++.
  \subsection{Máximo común divisor de una colección de enteros}
    El cálculo del máximo común divisor de una lista de enteros es una operación usual a la hora de trabajar con polinomios,
    ya que determinados métodos requieren polinomios en su forma «primitiva», es decir, con coeficientes coprimos. Aunque
    desafortunadamente comparten nombre, este concepto no debe confundirse con el de polinomio generador de un grupo
    multiplicativo, definición \ref{def:polyprim} de la página \pageref{def:polyprim}.

    Gracias a que el cálculo del máximo común divisor es asociativo y conmutativo, es posible aplicar
   el método representado en la figura \ref{fig:gcdLists}.  
    \begin{figure}[h]
      \centering
      \includegraphics[width=0.65\textwidth,keepaspectratio]{gcdLists} 
      \caption{Cálculo del máximo común divisor de una lista}\label{fig:gcdLists}
    \end{figure}
    Se realiza una partición (equilibrada) de la lista de enteros, y cada hilo disponible procesa una parte.
    Se reitera este proceso sobre la lista formada por los resultados, hasta llegar al cálculo de máximo 
    común divisor de dos valores, que arroja el resultado global para el total de la lista. 

    Nótese que es posible acelerar el cálculo en base a que $\textrm{mcd}(u_1, \ldots,u_k, 1) = 1$ para cualquier
    valor de $u_1, \ldots, u_k$. Según \cite{knuth}\footnote{pág. 341}, esto es aplicable el $60 \%$ de las veces,
    para una lista de valores aleatoria. De hecho, no se incluye para este caso gráfica comparativa debido a la 
...
\section{Evaluación de polinomios}\label{parallel:evalPoly}
  ...
  que éste es el principio en el que se sostiene la regla de Horner. 
...
\section{Resumen}
La tabla \ref{tab:resumenPar} resume los resultados de ganancia y eficiencia para los métodos con comparativa
expuestos a lo largo del presente capítulo.
\begin{table}[!h]
  \centering 
    \begin{tabular}{|c|c|c|c|} 
    \hline
     \textbf{Método} & \textbf{Ganancia} & \textbf{Eficiencia}  \\
    \hline \hline

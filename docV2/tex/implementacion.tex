...
%    \cite{miproyecto}, sección 5.2.1). Sin embargo, este mecanismo 
  ...
  \subsection{El compilador como calculadora}\label{impl:templatemp}
    ...
    Esta técnica es normalmente relegada al plano de las curiosidades pero que como se verá a continuación
    ...
    Como su propio nombre indica, aunque no lo parezca, representa la máxima potencia de $10$ representable mediante
    el tipo de dato básico \texttt{Digit}. Por ejemplo, si \texttt{Digit} tiene una longitud de $32$ bits, esto
    se corresponde a $10^9$, ya que $\lfloor \lg_{10}( 2^{32} ) \rfloor = 9$. El exponente de $10$ en cuestión 
    --que en el ejemplo anterior se corresponde a $9$--, se encuentra almacenado en otra constante, 
    ...
    encontrar un uso práctico a una técnica que parecería que no tiene. Se va a calcular una potencia entera 
    ...
    Sin entrar en demasiados detalles, ya que se insiste en que esta es una técnica un tanto esotérica aparta del uso
    ...
    \ref{algpow:2} para el caso en el que \texttt{N} es $1$. Tal caso base representaría $B^1 = B$, lo que justifica
...
\end{lstlisting}
  ...
  \subsection{Uso de \textit{autowiring}}
    ...
    \subsubsection{El método \texttt{getFunc}}
      ...
      de la biblioteca. Por tanto, también será responsabilidad de ésta el hacerlas cumplir. 
      
      Garantizar el cumplimiento de la primera condición es posible, incluso en tiempo de compilación, 
      ...
      entender cómo también se fuerza su cumplimiento, aunque en esta ocasión el compilador sin necesidad
      de ninguna triquiñuela se encargará. Se prosigue pues a describir el funcionamiento interno
      ...
      \begin{description}
        \item[Punto $1$, identificación del tipo.] Mediante el uso del operador \texttt{typeid} 
              ...
              ser leída por un usuario humano, pero el mero hecho de que se garantice su unicidad
              la convierte en una forma unívoca para identificar tipos.
...
\end{lstlisting}
  ...
  \subsection{Protección de secciones críticas}
    ...
    \begin{description}
      ...
      siendo utilizada por un programa concurrente (por ejemplo, el servidor de la aplicación MPPLab, \ref{mpplab:server}).
    ...
    recurso compartido el cual no debe ser accedido concurrentemente por más de un hilo (definición \ref{def:seccioncritica},
        sección \ref{par:seccioncritica}). El recurso compartido en este caso es la estructura de datos
...
\end{lstlisting}
    ...
    \subsubsection{Sección crítica: protocolos de entrada/salida}
      ...
      de entrada y salida comunes. Sin embargo estos constan de dos componentes diferenciadas, 
      fruto del punto de vista del que actúan. Nótese que el incorporar ambas medidas no es redundante,
      ya que es posible tener un programa concurrente utilizando asimismo OpenMP.
      \paragraph{Desde la concurrencia}
        ...
        de la aplicación MPPLab, se utiliza un lock de la biblioteca pthreads para asegurar la exclusión
        mutua. 
      \paragraph{Desde el paralelismo}
        Desde el punto de vista de OpenMP, la exclusión mutua se garantiza mediante la directiva 
        \texttt{critical}, de nombre bastante ilustrativo. 
...
\section{El mundo de los $64$ bits}\label{impl:64bits}
  ...
  \texttt{Digit} y \texttt{SignedDigit} como de $64$ bits. Debido a la estricta estructuración en capas 
  y el uso intensivo de la programación genérica, no realizando asunciones en los tamaños de los tipos 
...
\section{Implementación de tipos}
  ...
  \subsection{Reales} \label{impl:reales}
    ...
    \subsubsection{Cálculo de la raíz cuadrada} \label{impl:sqrt}
      ...
      con un orden de convergencia cuádruple ilustrado en la ecuación \eqref{eq:sqrtFinal} de dicha sección, que
...
\end{lstlisting}
   ...
   En los siguientes puntos, las líneas mencionadas se refieren siempre al listado \ref{lst:sqrt}.
      ...
      truncada del real de la biblioteca en cuestión. El problema se traslada pues a como el actual \texttt{mpplas::R} 
      ...
      \subsubsection{Funciones trascendentes} \label{impl:trascendentes}
        ...
        \paragraph{Cálculo del logaritmo.}
          ...
          desprenden son claras: por una parte, el cálculo de $\log(2)$ (también denominada
          ``La constante logarítmica'', véase \cite{log2}) es un problema
          ampliamente tratado y existen métodos de gran eficiencia para su cálculo. 
          Más adelante se tratará este punto en más profundidad.
          Por otra parte, al cumplirse $0 < y \leq 1$, el cálculo de
          $\log{(y)}$ puede realizarse satisfactoriamente y con relativa
          efectividad utilizando la expansión de MacLaurin
          para la función logaritmo.


        \paragraph{La constante $\pi$.} Se ha utilizado el conocido método de Machin. %TODO
...
un anilla. Una vez que se sabe que \texttt{S} es efectivamente un anillo, la segunda línea verifica que éste cumple la propiedad de ser
conmutativo.
...
\end{lstlisting}
  ...
  \subsection{Matrices}\label{impl:matrices}
    ...
    \subsubsection{Descomposición $LU$}\label{sec:lu}
      ...
      La implementación en sí del método es sencilla, y resulta una reescritura con los símbolos adecuados
      de C++ del algoritmo en pseudocódigo \ref{alg:doolittle} en la página \pageref{alg:doolittle}.

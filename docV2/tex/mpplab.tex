...
\section{Desarrollo}
  ...
  \subsubsection{Gestión dinámica de datos de los clientes en tiempo de ejecución.}\label{par:runtimeData}
    ...
    se representa a \textit{grosso modo} el modo de funcionamiento de este almacén 
...
\end{lstlisting}
  ...
  En la línea \ref{alg:acquire}, se asegura la sección crítica con un  \textit{lock} de \textit{pthreads}, que se libera
  ...
  \subsection{\emph{Un} cliente}\label{cliente}
    ...
    \subsubsection{\textit{Bootstrapping} y autoescritura.}\label{boostrapping}
      ...
      \paragraph{¿Cómo manejar los resultados de los métodos?}
        De nuevo, la declaración generica de los métodos priva de toda 
...
\end{lstlisting}
  ...
  de Python \texttt{ConfigParser}\footnote{\cite{pythonlib}, sección 9,2}. 
    ...
    \subsubsection{Trazado de gráficas}
      ...
      posible el trazado de gráficas siguiendo la sintaxis del conocido paquete de cálculo simbólico
    ...
    \subsubsection{Actualización automática.}
      ...
      --en concreto MD5-- para evitar comparar este fichero de forma íntegra. Si se 
    ...
    \subsubsection{Multihilo.}
      Esto no resulta más que un reflejo de ser a su vez multihilo el servidor. La
      manera de explotar esta característica se expone en la sección \ref{runInParallel}.
    ...
    \subsubsection{Modelando el servidor}
      ...
      capa se encuentra implementada mediante en el fichero  \texttt{RPCServer.py} y hace
      uso de la técnica de los mix-ins (sección \ref{tech:mixins}) y el patrón singleton (sección \ref{sec:singleton})
...
\subparagraph{Obtención a submatrices.}
 La operación de acceso a una submatriz opera de la siguiente manera:
  \begin{description}
    \item[\texttt{getSubMatrix(rows=(inicio,paso,fin), cols=(inicio,paso,fin))}] Devuelve la submatriz formada por 
    las filas/columnas especificadas por $inicio$, $paso$ y $fin$. Ambas filas/columnas
    del interval son \emph{incluidas}. 
    
    Es posible omitir estos parámetros, pero «manteniendo el orden»; es decir, es posible omitir $fin$ y $paso$, pero no
    es posible omitir $paso$ y especificar $fin$. Los valores dados a estos tres parametros por omisión son $0$, $1$ y 
...
a filas.
       ...
       \begin{description}
        ...
        \item[\texttt{solveFor(b)}] Espera un vector columna $b$ y devuelve el vector columna $y$ tal que $M \times y = b$, 
...
\end{verbatim}
       ...
       allí donde se utilice un tipo entero, habría de utilizarse un real. Asimismo, los métodos
...
\end{lstlisting}
    ...
    \begin{description}
       ...
       a considerar en la toma de datos.
        ...
        que en este contexto, «hilo» se refiere a hilos utilizados por la biblioteca, no ejecuciones simultáneas de 

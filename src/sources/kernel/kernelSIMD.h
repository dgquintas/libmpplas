...
namespace mpplas{
  ...
  inline std::ostream& operator<<(std::ostream& out, const SIMDDigit<float4xSIMD_t>& v){
    float x[4] __attribute__((aligned(16)));
    SIMDCPU::Unpack(x,v);
    out << x[0] << " " << x[1] << " " << x[2] << " " << x[3];
    return out;
  }
  inline std::ostream& operator<<(std::ostream& out, const SIMDDigit<double2xSIMD_t>& v){
  ...
  inline std::ostream& operator<<(std::ostream& out, const SIMDDigit<int8xSIMD_t>& v){

...
R numth::operator+(R izq, const R& der)
{
  izq += der;
  return izq;
}
...
R numth::operator+(const double corto, R largo)
{
  largo += corto;
  return largo;
}
...
std::ostream& 
numth::operator<<(std::ostream& out, R numero)
{
...
std::istream& 
numth::operator>>(std::istream& in, R& numero)
{
  std::string entrada;
  Cifra num;
  char *error;
...
R numth::operator-(R real)
{
  real.cambiarSigno();
  return real;
}

R numth::operator^(R base, const Z& exp)
{
  if( exp.longitud() > 1 )
    throw Errores::DemasiadoGrande();
...
R numth::operator^(R base, CifraSigno exp)
{
  base ^= exp;
  return base;
}

R numth::operator>>(R real, const size_t n)
{
  real >>= n;
  return real;
}

...
\section{Programación orientada a aspectos (AOP)}\label{tech:aop}
  Este relativamente novedoso paradígma es considerado por algunos como una evolución de la programación
  ...
  cosas como «para cada método cuyo nombre comiece con 'test', añadirlo a la lista de métodos a ejecutar
  ...
  a las ofrecias por la programación orientada a objetos. Más información acerca de este paradigma puede, entre
...
\section{\textit{Mixins}} \label{tech:mixins}\index{mixin}{mixin}
  ...
  Si se considera un languaje con soporte de interfaces, tal como Java, podría pensarse que
  estos son equivalentes a las clases mix-in. Sin embargo, por definición, un interfaz no lleva
...
\end{lstlisting}
  ...
  El detalle más importante a observar la sección crítica, allí donde se modifica el dato estático \texttt{\_instance}, que se 
  extiende de las líneas \ref{algsingleton:cs1} a \ref{algsingleton:cs2}. Por tanto, en un marco concurrente, debe asegurarse la
  dicha sección exclusión mutua dentro de dicha sección crítica. Esto desemboca en el uso de \textit{locks}, de tal forma que cada vez
  ...
  como el intento de adquirir el \textit{lock}. Frente a esto se desarrolló otro patrón más complejo, bautizado como \textit{double-checked
  ...
  sus comprobaciones extras y se asume la perdida de rendimiento «por necesidad del guión» o bien... volviendo atrás sobre nuestros pasos,
  ...
  en el momento la construcción, sino que se distribuye a lo largo del tiempo. Si el objeto a construir es complejo, esta técnica realmente
...
\section{\textit{Autowiring}}\label{sec:autowiring}
    Detrás de otro vago termino anglosajón, se esconde la idea de asociar dos elementos automáticamente
    ...
    La idea deriva de un mecanismo similar implementado en el popular framework para Java Spring\footnote{
      \url{http://static.springframework.org/spring/docs/2.5.x/reference/beans.html\#beans-factory-autowire}}.
...
\section{Pruebas unitarias}\label{testunits}
  ...
  Este tipo de pruebas son el hilo conductor de \cite{testdrivendevelopment}, del mismo autor y uno de los pilares
  de la filosofía de programación conocida como «extreme programming»\footnote{que no consiste
    en desarrollar mientras se salta de un avión, sino que se ha probado muy efectiva en numerosos ámbitos. Véase
  ...
  Este tipo de pruebas aportan las siguientes ventajas:
    ...
    \begin{description}
      ...
      funcionando correctamente, de incentiva el realizar cambios para mejorar el código.
    \end{description}
  ...
  no han sido desarrollados de nuevo, sino que se ha utilizado el paquete QtUnit
  \footnote{\url{http://www.uwyn.com/projects/qtunit}}, de forma totalmente satisfactoria. 
  ...
  \subsection{Necesidad y uso de un «oráculo»}\label{tech:oraculo}
   ...
   de entrada y salida válidos y/o inválidos para un método dado. 
   ...
   gran cantidad de métodos es potencialmente infinito, lo cual no deja de ser precisamente
   la utilidad de la misma: la precisión arbitrariamente grande de los datos manejados.
   Ante lo limitado de considerar una serie de pares entrada/salida, generados de antemano
   de tal forma que se garantiza su corrección, se ha optado por recurrir a un \emph{oráculo}\index{oráculo}{oráculo}. 
   ...
   Si se dispone de un tal oráculo capaz de proporcionar una respuesta para el par método-entradas,
   sería posible automatizar aún más la ejecución de pruebas con un conjunto de entradas mucho mayor,
   al poder generar las entradas de forma aleatoria. Estas entradas aleatorias serían 
   ...
   al formato propio del oráculo en cuestión. Y por supuesto, que un oráculo como el descrito
...
El concepto de \textit{mock object}\footnote{podría traducirse como «objecto simulado», aunque incluso en la
  ...
  baterías de pruebas
  basadas en pruebas unitarias (véase sección \ref{testunits} al respecto). 

  En el caso de esta biblioteca resulta ventajosa la utilización de estos objetos es en lo relacionado con las rutinas 
 ...
 \section{Generación dinámica de código}
  ...
  La implementación de estos mecanismos requiere en ocasiones un uso poco ortodoxo o directamente 
  ...
  \subsection{Herramientas de soporte}
    ...
    es asimismo uno de los mecanismos más complejos y que más sorpresas, para bien o para mal, 
    da al programador. 
  ...
  \subsection{Mecanismo de perfilado basado en AOP}\label{tech:aop}
    \subsubsection{Preprocesando con Python}

\section{Para el desarrollo}
  A lo largo del proceso de desarrollo, se han utilizado algunas herramientas habituales cuando
el proyecto es de una cierta envergadura. El presente proyecto cuenta aproximadamente con $32000$ líneas
...
resultado útil. De nuevo, ¡qué trabaje la máquina!
  ...
  \subsection{Control de versiones}\label{tech:svn}
    ...
    y --sobre todo-- el seguimiento de los cambios. No es desdeñable tampoco la capacidad de deshacer un cambio, volviendo
  ...
  \subsection{Gestión de la compilación}\label{tech:scons}
    Si la gestión de las versiones resulta una pesada carga cuando se trabaja con las cifras presentadas
    ...
    \subsubsection{Autoajuste al entorno}
      ...
      \paragraph{Verificando el entorno.} El código desarrollado descansa sobre algunas suposiciones. Se ha
        ...
        en su sección 7.18.1.1. En el caso del compilador GCC, se soportan estos tipos desde la versión %TODO.
